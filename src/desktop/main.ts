...
function diagnosticsHtml(state: ManagerSnapshot): string {
  ...
  const keyboardMode = state.device?.connection === "USB-KBD/HID" ? `<div class="card"><strong>Modo teclado USB</strong><p>El lector está disponible. Las lecturas llegan directamente a la ventana de Kontave que esté en primer plano; Device Manager no intercepta teclas de otras aplicaciones.</p></div>` : "";
  return `<!doctype html><html lang="es"><meta charset="utf-8"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline'; img-src data:"><link rel="icon" href="${favicon}"><title>Kontave Device Manager</title><style>body{font:14px system-ui;margin:0;background:#f5f7fb;color:#162033}.head{background:#0b0c14;color:white;padding:24px;display:flex;align-items:center;gap:14px}.brand{width:44px;height:44px}.head h2{margin:0}.head div div{margin-top:4px;color:#c6c8d2}main{padding:24px}.card{background:white;border:1px solid #dde3ed;border-radius:12px;padding:18px;margin-bottom:14px}.card p{margin:8px 0 0;color:#667085;line-height:1.45}.status{font-weight:700;text-transform:capitalize}.ok{color:#16803c}.bad{color:#b42318}dt{color:#667085;margin-top:12px}dd{margin:3px 0 0;font-weight:600}.foot{color:#667085;font-size:12px}</style><div class="head"><img class="brand" src="${favicon}" alt=""><div><h2>Kontave Device Manager</h2><div>Centro local de dispositivos</div></div></div><main><div class="card"><div class="status ${state.status === "connected" ? "ok" : "bad"}">${escape(state.status)}</div><dl><dt>Dispositivo</dt><dd>${escape(state.device ? `${state.device.manufacturer} ${state.device.model}` : null)}</dd><dt>Conexión</dt><dd>${escape(state.device?.connection)}</dd><dt>Gateway</dt><dd>${escape(state.gatewayUrl)}</dd><dt>Último error</dt><dd>${escape(state.lastError)}</dd></dl></div>${keyboardMode}<p class="foot">La aplicación continúa funcionando al cerrar esta ventana. Usa el icono de la bandeja para salir.</p></main></html>`;
}
