...
export class QW2100Adapter implements DeviceAdapter {
  ...
  private matches(port: PortInfo): boolean { if (this.config.serialPort) return port.path === this.config.serialPort; if (this.config.serialNumber) return port.serialNumber === this.config.serialNumber; if (this.config.vendorId || this.config.productId) return (!this.config.vendorId || normalize(port.vendorId) === normalize(this.config.vendorId)) && (!this.config.productId || normalize(port.productId) === normalize(this.config.productId)); return Boolean(port.vendorId && port.productId && !/bluetooth/i.test(`${port.manufacturer ?? ""} ${port.pnpId ?? ""}`)); }
